...
import { AppConfig, Networks, NetworksConfig, Wallets } from './types'
import { messagelib, protocol } from '@e2e/typechain'
import DVN__factory = messagelib.factories.uln.dvn._DVN__factory.DVN__factory
...
const createNetworks = (): Networks => {
    ...
    for (const network in networksConfig) {
        ...
        const selfAddress = read.readDevloyments(network, 'DVN').address
        const self = DVN__factory.connect(selfAddress, provider)
        ...
        const wallets: Wallets = {
            admin: new ethers.Wallet(networksConfig[network].keys.adminPrivateKey, provider),
            signers: networksConfig[network].keys.signerPrivateKeys.map((key) => new ethers.Wallet(key, provider)),
        }
